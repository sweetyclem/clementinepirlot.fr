...
const loadResult = () => {
    let container = document.getElementById(`quiz-main-page`)
    // const answers = quiz.questions.flatMap(q => q.entered)
    let answers = []
    quiz.questions.forEach(question => {
      answers.push(question.answers[question.entered])
    })
    let resultDiv = document.createElement(`div`);
    resultDiv.className = `form-control result`;
    resultDiv.innerText = `You answered ${answers}`
